# Screenshots checklist

Capture these for the written submission.

- Production VM PetClinic welcome screen before the demo change
- Production VM PetClinic welcome screen after the demo change
- Jenkins pipeline run showing all stages
- Jenkins console output showing Maven build and tests
- Jenkins published Security Report artifact
- SonarQube Spring PetClinic quality gate
- Burp Suite Proxy HTTP history for the PetClinic target
- Prometheus `/targets` with Jenkins, Prometheus, and PetClinic
- Prometheus `/alerts` with Jenkins and PetClinic alert rules
- Grafana Prometheus datasource
- Grafana Jenkins dashboard
- Git commit or Jenkins build page proving the code-change-triggered run

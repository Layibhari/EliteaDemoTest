...
class OwnerControllerTests {
	...
	@Test
	void processFindByLastNameEndpointSuccess() throws Exception {
		Owner george = george();
		Page<Owner> page = new PageImpl<>(List.of(george));

		// Simula búsqueda solo por apellido
		when(this.owners.findByLastNameStartingWith(eq("Franklin"), any(Pageable.class))).thenReturn(page);

		mockMvc.perform(get("/owners/searchByLastName").param("lastName", "Franklin"))
			.andExpect(status().is3xxRedirection())
			.andExpect(view().name("redirect:/owners/" + TEST_OWNER_ID));
	}

	@Test
	void processFindByLastNameEndpointNoResults() throws Exception {
		...
		mockMvc.perform(get("/owners/searchByLastName").param("lastName", "Unknown"))
			.andExpect(status().isOk())
			.andExpect(model().attributeHasFieldErrors("owner", "lastName"))
			.andExpect(model().attributeHasFieldErrorCode("owner", "lastName", "notFound"))
			.andExpect(view().name("owners/findOwners"));
	...
	@Test
	void showAllOwnersEndpointSuccess() throws Exception {
		Owner george = george();
		Page<Owner> page = new PageImpl<>(List.of(george, new Owner(), new Owner()));

		when(this.owners.findAll(any(Pageable.class))).thenReturn(page);

		mockMvc.perform(get("/owners/showAll").param("page", "1"))
			.andExpect(status().isOk())
